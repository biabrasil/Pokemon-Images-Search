...
function ProfileCard({ title, handle, image }) {
  ...
  return (
    <div onClick={handleClick} className={'inline-block mx-5 mt-5'}>
      ...
      <div className={'rounded-b-lg shadow-md w-full max-w-full bg-pink-200 py-6 flex flex-wrap items-center justify-center'}>
          <img alt="heart" src={Heart} style={{width:50 + 10 * clicks + 'px'}} className={'absolute'} />
          </div>
    </div>
  );
}
